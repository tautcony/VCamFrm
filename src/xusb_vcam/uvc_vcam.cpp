...
void uvc_vcam::fill_str_desc(const char* manu_fact, const char* product)
{
    ...
    auto len = wcslen((PWCHAR)str->wData) * sizeof(WCHAR);
    str->bLength = (__u8)(2 + len);
...
void uvc_vcam::fill_cfg_desc()
{
    ...
    cfg_desc->wTotalLength = (__le16)(((char*)ep + ep->bLength) - cfg_buffer);
}
...
void uvc_vcam::iso_transfer(usbtx_header_t* hdr)
{
    ...
    for (int i = 0; i < (int)iso_hdr->number_packets - 1; ++i) {
