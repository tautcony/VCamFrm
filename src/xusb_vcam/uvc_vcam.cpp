...
protected:
    ...
    {
        uvc_vcam* uvc = static_cast<uvc_vcam*>(_p);
        uvc->loop();
        return 0;
    }
...
{
    auto str = reinterpret_cast<usb_string_descriptor*>(str_buffer);
    str->bLength = 0x04;
    str->bDescriptorType = 0x03; // string
    str->wData[0] = 0x0409; // english
    str_desc[0] = str;

    str = reinterpret_cast<usb_string_descriptor*>(reinterpret_cast<char*>(str) + str->bLength); // manu_fact
    str->bDescriptorType = 0x03;
    MultiByteToWideChar(CP_ACP, 0, manu_fact, -1, reinterpret_cast<PWCHAR>(str->wData), 256);
    auto len = wcslen(reinterpret_cast<PWCHAR>(str->wData)) * sizeof(WCHAR);
    str->bLength = static_cast<__u8>(2 + len);
    str_desc[1] = str; // iManufacturer

    str = reinterpret_cast<usb_string_descriptor*>(reinterpret_cast<char*>(str) + str->bLength);
    str->bDescriptorType = 0x03;
    MultiByteToWideChar(CP_ACP, 0, product, -1, reinterpret_cast<PWCHAR>(str->wData), 256);
    len = wcslen(reinterpret_cast<PWCHAR>(str->wData)) * sizeof(WCHAR);
    str->bLength = static_cast<__u8>(2 + len);
...
{
    // 配置描述符
    cfg_desc = reinterpret_cast<usb_config_descriptor*>(cfg_buffer);
    ...
    // IAD Descriptor
    const auto iad = reinterpret_cast<usb_interface_assoc_descriptor*>(reinterpret_cast<char*>(cfg_desc) + cfg_desc->bLength);
    ...
    auto intr = reinterpret_cast<usb_interface_descriptor*>(reinterpret_cast<char*>(iad) + iad->bLength);
    ...
    const auto vchdr = reinterpret_cast<uvc_header_descriptor*>(reinterpret_cast<char*>(intr) + intr->bLength);
    ...
    const auto ctd = reinterpret_cast<uvc_camera_terminal_descriptor*>(reinterpret_cast<char*>(vchdr) + vchdr->bLength);
    ...
    ctd->bAssocTerminal = 0x00; // 不关联到 output terminal descriptor
    ...
    const auto pud = reinterpret_cast<uvc_processing_unit_descriptor*>(reinterpret_cast<char*>(ctd) + ctd->bLength);
    ...
    const auto otd = reinterpret_cast<uvc_output_terminal_descriptor*>(reinterpret_cast<char*>(pud) + pud->bLength);
    ...
    const auto eud = reinterpret_cast<uvc_extension_unit_descriptor_1_3*>(reinterpret_cast<char*>(otd) + otd->bLength);
    ...
    static const GUID g = {0x88f04da6, 0xf713, 0x454a, {0xb6, 0x25, 0x37, 0x93, 0xae, 0x44, 0x75, 0x16}};
    ...
    intr = reinterpret_cast<usb_interface_descriptor*>(reinterpret_cast<char*>(eud) + eud->bLength);
    intr->bLength = 0x09; // Length
    ...
    const auto vshdr = reinterpret_cast<uvc_input_header_descriptor*>(reinterpret_cast<char*>(intr) + intr->bLength);
    ...
    const auto fmt = reinterpret_cast<uvc_format_uncompressed*>(reinterpret_cast<char*>(vshdr) + vshdr->bLength);
    ...
    // frames
    auto frame = reinterpret_cast<uvc_frame_uncompressed*>(reinterpret_cast<char*>(fmt) + fmt->bLength);
    for (int i = 0; i < frame_count; ++i)
    {
        ...
        frame = reinterpret_cast<uvc_frame_uncompressed*>(reinterpret_cast<char*>(frame) + frame->bLength);
    }

    // VS Color Matching Descriptor Descriptor
    const auto cmd = reinterpret_cast<uvc_color_matching_descriptor*>(frame);
    ...
    intr = reinterpret_cast<usb_interface_descriptor*>(reinterpret_cast<char*>(cmd) + cmd->bLength);
    intr->bLength = 0x09;
    ...
    // Endpoint Descriptor
    const auto ep = reinterpret_cast<usb_endpoint_descriptor*>(reinterpret_cast<char*>(intr) + intr->bLength);
    ...
    //Bits 10..00 : 0x400 (1024 bytes per packet)
    ep->bInterval = 0x01;

    cfg_desc->wTotalLength = static_cast<__le16>((reinterpret_cast<char*>(ep) + ep->bLength) - cfg_buffer);
}
...
void uvc_vcam::loop()
{
    while (!quit)
    {
        const auto hdr = virt_usb_begin(usb_ptr);
        if (!hdr)continue;
        ...
        {
            // 数据传输
            if (hdr->transfer.type == 3)
            {
                // 同步传输
                // printf("iso transfer\n");
                iso_transfer(hdr);
            }
            else if (hdr->transfer.type == 1)
            {
                // 控制传输，处理某些请求描述符直接采用控制传输请求
        ...
        {
            // unknown request
            printf("unknown usb request type=%d, subtype=%d\n", hdr->type, hdr->descriptor.type);
        }
...
{
    if (hdr->descriptor.type == 1)
    {
        // 获取描述符 ,hdr->descriptor.is_read ==TRUE
        if (hdr->descriptor.subtype == 1)
        {
            // 设备描述符
            hdr->result = 0;
        ...
        else if (hdr->descriptor.subtype == 2)
        {
            // 配置描述符
            if (hdr->data_length < 0x09)
            {
                printf("fault err cfg_desc length not match\n");
                hdr->result = -1;
            }
            else
            {
        ...
        else if (hdr->descriptor.subtype == 3)
        {
            // 获取字符串
            ...
            {
                hdr->result = 0;
                // 这里没对原来的data_length判断，认为都有足够缓存来接纳字符串
                hdr->data_length = str_desc[hdr->descriptor.index]->bLength;
                memcpy(hdr->data, str_desc[hdr->descriptor.index], hdr->data_length);
            }
        }
    }
    else if (hdr->descriptor.type == 2)
    {
        // 设置配置或者接口描述符
        if (hdr->descriptor.subtype == 1)
        {
    ...
    {
        printf("unknown descriptor request type=%d, subtype=%d\n", hdr->descriptor.type, hdr->descriptor.subtype);
    }
}
...
void uvc_vcam::vendor_control(usbtx_header_t* hdr)
{
    printf("vender type=%d,subtype=%d, request=0x%X, index=0x%X, value=0x%X,LEN=%d\n",
           hdr->vendor.type, hdr->vendor.subtype, hdr->vendor.request, hdr->vendor.index, hdr->vendor.value,
           hdr->data_length);
    ...
    case UVC_GET_DEF:
        hdr->result = 0;
        memset(hdr->data, 0, hdr->data_length);
        break;

    case UVC_GET_CUR:
        {
            if (LOBYTE(hdr->vendor.index) == 1)
            {
                // 视频流接口
                if (hdr->data_length < 26)
                {
                    printf("UVC 1.0  GET_CUR must use 26 bytes \n");
                    break;
                }

                const auto sc = reinterpret_cast<uvc_streaming_control*>(hdr->data);
                ...
                sc->dwMaxVideoFrameSize = frames[curr_frame_index - 1].width * frames[curr_frame_index - 1].height * (
                    bitsPerPixel / 8);
    ...
    case UVC_SET_CUR:
        {
            if (LOBYTE(hdr->vendor.index) == 1)
            {
                ...
                if (hdr->data_length < 26)
                {
                    printf("UVC 1.0  SET_CUR must use 26 bytes \n");
                    break;
                }
    ...
    case UVC_GET_MIN:
        {
            if (LOBYTE(hdr->vendor.index) == 1)
            {
                ...
                if (hdr->data_length < 26)
                {
                    printf("UVC 1.0  GET_MIN must use 26 bytes \n");
                    break;
                }
    ...
    case UVC_GET_MAX:
        {
            if (LOBYTE(hdr->vendor.index) == 1)
            {
                ...
                if (hdr->data_length < 26)
                {
                    printf("UVC 1.0  GET_MAX must use 26 bytes \n");
                    break;
                }
...
{
    const auto code = hdr->transfer.setup_packet;
    const auto value = *reinterpret_cast<unsigned short*>(&code[2]);
    const auto index = *reinterpret_cast<unsigned short*>(&code[4]);
    ...
    else if (code[1] == 8)
    {
        ...
        printf("control transfer not supported GET_CONFIG\n");
    }
    else if (code[1] == 11)
    {
        // SET_INTERFACE 设置接口
        const int intfNum = LOBYTE(index);
        const int altSet = LOBYTE(value);
        hdr->result = 0;
    ...
    else if (code[1] == 1)
    {
        // CLEAR_FEATURE
        hdr->result = 0;
        printf("control transfer CLEAR_FEATURE \n");
    }
    else if (code[0] == 0xA1)
    {
        // CLASS 请求
        hdr->type = 2;
        hdr->vendor.type = 1; // CLASS
...
{
    const auto iso_hdr = reinterpret_cast<iso_packet_hdr*>(hdr->data);
    const int HDR_LEN = ISO_PACKET_HDR_SIZE + iso_hdr->number_packets * sizeof(iso_packet);
    const auto buffer = hdr->data + HDR_LEN;
    const auto buf_len = hdr->data_length - HDR_LEN;
    if (buf_len <= 12)
    {
        // 如果长度小于playerheader头
        printf("fault error\n");
    ...
    const auto frmlen = (bitsPerPixel / 8) * frames[curr_frame_index - 1].width * frames[curr_frame_index - 1].height;
    ...
    if (frame_length != frmlen || !frame_buffer)
    {
        ...
        frame_pos = 0;
        if (frame_buffer)free(frame_buffer);
        frame_buffer = static_cast<char*>(malloc(frame_length));
    }
    ...
    if (frame_pos <= 0)
    {
        ...
        const auto r = frame_callback(&frame);
        if (r < 0)
        {
            return;
        }
    ...
    for (auto i = 0; i < static_cast<int>(iso_hdr->number_packets) - 1; ++i)
    {
        const auto f_pkt = &iso_hdr->packets[i];
        const auto n_pkt = &iso_hdr->packets[i + 1];
        int dt = n_pkt->offset - f_pkt->offset;
        auto r = video_encode(reinterpret_cast<unsigned char*>(buffer) + f_pkt->offset, dt);
        ...
        if (static_cast<unsigned>(i) == iso_hdr->number_packets - 2 && ret < buf_len)
        {
            dt = n_pkt->offset - f_pkt->offset;
            dt = min(buf_len - ret, dt);

            r = video_encode(reinterpret_cast<unsigned char*>(buffer) + n_pkt->offset, dt);
            if (r <= 0)continue;
            ret += r;
            n_pkt->length = r;
            n_pkt->status = 0;
...
int uvc_vcam::video_encode(unsigned char* vid_buf, const int vid_len)
{
    const auto VID_HDR_LEN = 2;

    const auto vid_data = vid_buf + VID_HDR_LEN;
    const auto data_len = vid_len - VID_HDR_LEN;

    const auto vid_hdr = vid_buf;
    ...
    const auto len = min(frame_length - frame_pos, data_len);
...
int uvc_vcam::create(uvc_vcam_t* uvc)
{
    ...
    char dev_id[256];
    sprintf(dev_id, "usb\\vid_%.4x&pid_%.4x", uvc->vid, uvc->pid);
    ...
    const auto r = virt_usb_plugin(usb_ptr, dev_id, dev_id, "USB\\DevClass_00&SubClass_00\nUSB\\DevClass_00\nUSB\\COMPOSITE");
    if (r < 0)
    {
        virt_usb_close(usb_ptr);
        usb_ptr = nullptr;
    ...
    DWORD tid;
    hThread = CreateThread(nullptr, 0, __thread, this, 0, &tid);
...
{
    auto vcam = new uvc_vcam;
    const auto r = vcam->create(uvc);
    if (r < 0)
    {
        delete vcam;
        return nullptr;
...
{
    const auto vcam = static_cast<uvc_vcam*>(handle);
    if (!vcam)return;
